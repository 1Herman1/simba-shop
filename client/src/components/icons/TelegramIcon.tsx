/** Фирменный самолётик Telegram (FontAwesome telegram-plane path) — единственная
    версия на сайте. Раньше в разных местах жили разные дженерик-путя разного
    оптического веса (см. design-reviewer). */
export default function TelegramIcon({ className }: { className?: string }) {
  return (
    <svg width="20" height="20" viewBox="0 0 512 512" fill="currentColor" aria-hidden="true" className={`w-5 h-5 flex-shrink-0 ${className ?? ''}`}>
      <path d="M446.7 98.6l-67.6 318.8c-5.1 22.5-18.4 28.1-37.3 17.5l-103-75.9-49.7 47.8c-5.5 5.5-10.1 10.1-20.6 10.1l7.4-104.9L366 132.9c8.3-7.4-1.8-11.5-12.9-4.1L117.8 284 16.2 252.2c-22.1-6.9-22.5-22.1 4.6-32.7L418.2 66.4c18.4-6.9 34.5 4.1 28.5 32.2z" />
    </svg>
  )
}
